...
export function IOSFilterSheet({ open, onClose }: IOSFilterSheetProps) {
  ...
  const showSearchFilters = viewMode === "search" || !!searchTagId;
  ...
  return (
    <IOSSheet open={open} onClose={handleClose} title="表示オプション" tall>
      <div className="space-y-6 p-4">
        ...
        <section>
          <h3 className="ios-section-header mb-2 px-1 text-xs uppercase text-neutral-500">検索タグ</h3>
          <TagSearchPicker
            variant="ios"
            allTags={allTags}
            selectedTagId={searchTagId}
            onSelect={setSearchTagId}
          />
        </section>
